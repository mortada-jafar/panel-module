...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <!-- BEGIN: Data List -->
        @include('panel_ui::common.loading')
    ...
    @push('scripts')
        <script>
            ...
            const optTranslate = {
                "fa": {
                    like: "{{ __('panel_ui::panel.contains') }}",
                    in: "{{ __('panel_ui::panel.contains') }}",
                    nlike: "{{ __('panel_ui::panel.ncontains') }}",
                    eq: "{{ __('panel_ui::panel.equals') }}",
                    neqs: "{{ __('panel_ui::panel.nequals') }}",
                    gt: "{{ __('panel_ui::panel.greater') }}",
                    lt: "{{ __('panel_ui::panel.less') }}",
                    gte: "{{ __('panel_ui::panel.greatere') }}",
                    lte: "{{ __('panel_ui::panel.lesse') }}",
                },
                "en": {
                    like: "like",
                    nlike: "not like",
            ...
            function getFilterTag($filter) {
                return `<div data-id="${$filter.id}" data-filter-start="${$filter.val}" class="bg-theme-white  rounded mt-1 me-2  overflow-hidden shadow">
                                <span class="text-white bg-theme-primary py-2 px-2 float-bottom">${$filter.label} ${optTranslate['fa'][$filter.cond]} :</span><span
                                    class="text-theme-primary py-2 px-2 float-bottom">${$filter.option === "option" || $filter.type === "datetime" ? $filter.textVal : $filter.val}</span>
                ...
                $('#add-filter').click(function () {
                    ...
                    if (filterType === "options") {
                        ...
                        if (filterType=="datetime") {
                            filter.val = $('#datepicker-alt').val();
                            filter.textVal = $('#datepicker').val();
                        } else {
